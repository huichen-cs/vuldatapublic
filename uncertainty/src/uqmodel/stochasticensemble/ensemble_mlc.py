...
class StochasticEnsembleClassifier(object):
    ...
    """
    def __init__(self, Classifier, log_sigma:bool, ensemble_size:int, input:int, output:int=2,
                 neurons:Sequence=[300, 300, 300], dropouts:Sequence=[0.25, 0.25, 0.25], activation=None):
        self.size = ensemble_size
        self.model_ensemble = [Classifier(input, log_sigma, output, neurons, dropouts, activation=activation) for i in range(ensemble_size)]
...
class EnsembleClassifier(object):
    ...
    """
    def __init__(self, ensemble_size:int, input:int, output:int=2,
                 neurons:Sequence=[300, 300, 300], dropouts:Sequence=[0.25, 0.25, 0.25], activation=None):
        self.n_outputs = output
        self.size = ensemble_size
        self.model_ensemble = [MultiLayerClassifier(input, output, neurons, dropouts, activation) for i in range(ensemble_size)]
    ...
    def predict_logits(self, data_loader:torch.utils.data.DataLoader):
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                batch_logits = torch.zeros(self.size, batch_size, output_size).to(device)
                x = x.to(device)
