...
if [ -z ${runid} ]; then
  echo "runid is unset"
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext.sh 5 random; then
    echo "Failed to run random."
    exit 1
  fi
else
  echo "runid is set to '${runid}'"
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext.sh 5 random "${runid}"; then
    echo "Failed to run random with runid '${runid}'."
    exit 1
  fi
fi
