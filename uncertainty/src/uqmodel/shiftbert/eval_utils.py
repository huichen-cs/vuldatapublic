    ...
    def predict_proba(
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                ...
                input_ids = input_ids.to(device)
                attention_mask = attention_mask.to(device)
                dropout_proba, mean_proba, confidence, labels = dropout_model.predict(
                    input_ids, attention_mask
                )
...
class VanillaBertClassifierEvalautor(object):
    ...
    def compute_eval_metrics(
        ...
        dataloader = self.dataloader
        logger.debug("predicting proba and confidence ...")
        proba_list, confidence_list, labels_list = self.predict_proba(
            dataloader, device
        )
        logger.debug("computing predictive performance ...")
        ...
        ece = compute_ece(proba_pred, targets).cpu().item()
        score = (
            brier_score(get_one_hot_label(targets, num_classes=2), all_proba)
            .cpu()
            .item()
        )
        conf_thresholds = torch.linspace(0, 10, 11) * 0.1
