...
class BertExperimentDatasets(object):
    ...
    def __init__(self, config:ExperimentConfig, tag:Union[str, None], seed:int=1432, active_learn:bool=False):
        self.config = config
    ...
    def _generate_datasets(self):
        sap_data = SapData(self.config.data.data_dir)
        ...
        tokenizer = transformers.AutoTokenizer.from_pretrained("microsoft/codebert-base", cache_dir=self.config.cache_dir)
        train_dataset = TextClassificationDataset(
            data_splits['train'], tokenizer,
            self.config.model.max_encoding_len,
            self.config.model.num_classes)
        val_dataset = TextClassificationDataset(
            data_splits['val'], tokenizer,
            self.config.model.max_encoding_len,
            self.config.model.num_classes)
        test_dataset = TextClassificationDataset(data_splits['test'], tokenizer,
            self.config.model.max_encoding_len,
            self.config.model.num_classes)
    ...
    def build_datasets(self):
        if self.config.trainer.use_data == 'use_checkpoint':
            try:
                ...
                logger.info('loaded train/val/test datasets from checkpoint at {}'
                            .format(self.config.trainer.checkpoint.dir_path))
            except FileNotFoundError as err:
                logger.info('unable to load checkpoint, prepare data sets '
                            +
                            'with train/test ratios: {} and validation ratio: {}'
                            .format(self.config.train_test_ratios, self.config.val_ratio))
                raise err
            ...
            try:
                ...
                logger.info('unable to load checkpoint, prepare data sets '
                            + 
                            'with train/test ratios: {} and validation ratio: {}'
                            .format(self.config.data.train_test_ratios,
                                    self.config.data.val_ratio))
            ...
            raise ValueError('unsupported configuration option {} for config.trainer.use_data'
                             .format(self.config.trainer.use_model))
        return  train_dataset, val_dataset, test_dataset
