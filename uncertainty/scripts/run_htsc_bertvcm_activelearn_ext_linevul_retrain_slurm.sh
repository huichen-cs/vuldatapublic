...
if [ $# -lt 3 ]; then
  echo "Usage: $0 model_idx active_learn_method infer_batch_size"
  echo "Usage: $0 model_idx active_learn_method infer_batch_size runid"
  echo "Usage: $0 model_idx active_learn_method infer_batch_size runid batchsize"
  exit 1
fi
...
## command line argument - infer_batch_size
if [ $# -gt 2 ]; then
  infer_batch_size=$3
  echo "Got infer_batch_size ${infer_batch_size} from the command line"
fi
...
if [ -z "${runid}" ]; then
  echo "runid is unset"
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext_linevul_retrain.sh \
    "${model_idx}" "${method}" "${infer_batch_size}"; then
    echo "Failed to run run_htsc_bertvcm_activelearn_ext_linevul.sh"
    echo "model_idx: ${model_idx}, method: ${method}, infer_batch_size: ${infer_batch_size}"
    exit 1
  fi
elif [ -n "${runid}" ] && [ -z "${batchsize}" ]; then
  echo "runid is set to '${runid}'"
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext_linevul_retrain.sh \
    "${model_idx}" "${method}" "${infer_batch_size}" "${runid}"; then
    echo "Failed to run run_htsc_bertvcm_activelearn_ext_linevul.sh"
    echo "model_idx: ${model_idx}, method: ${method}, infer_batch_size: ${infer_batch_size}, runid: ${runid}"
    exit 1
  fi
  ...
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext_linevul_retrain.sh \
    "${model_idx}" "${method}" "${infer_batch_size}" "${runid}" "${batchsize}"; then
    echo "Failed to run run_htsc_bertvcm_activelearn_ext_linevul.sh"
    echo -n "model_idx: ${model_idx}, method: ${method}, infer_batch_size: ${infer_batch_size}, "
    echo "runid: ${runid}, batchsize: ${batchsize}"
    exit 1
  fi
else
  echo "runid is set to '${runid}'"
  ...
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext_linevul_retrain.sh \
    "${model_idx}" "${method}" "${infer_batch_size}" "${runid}" "${batchsize}" "${linevul_input_file}"; then
    echo "Failed to run run_htsc_bertvcm_activelearn_ext_linevul.sh"
    echo -n "model_idx: ${model_idx}, method: ${method}, infer_batch_size: ${infer_batch_size}, "
    echo "runid: ${runid}, batchsize: ${batchsize}, linevul_input_file: ${linevul_input_file}"
    exit 1
  fi
fi
