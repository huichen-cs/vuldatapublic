...
class DropoutClassifier(object):
    ...
    def predict_logits(
        self, data_loader: torch.utils.data.DataLoader, n_stochastic_passes: int
    ):
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                batch_logits = torch.zeros(
                    n_stochastic_passes, batch_size, output_size
                ).to(device)
                x = x.to(device)
    ...
    def predict_logits_proba(
        self, data_loader: torch.utils.data.DataLoader, n_stochastic_passes
    ):
    ...
    def predict_from_dropouts(self, test_dataloader, n_samples=1000, device=None):
        with torch.no_grad():
            for test_batch in test_dataloader:
                ...
                samples_logits_batch = torch.stack(samples_logits, dim=1).transpose(
                    0, 1
                )
                ...
                (
                    predicted_class_samples_proba,
                    predicted_class_samples_batch,
                ) = torch.max(proba_samples_mean_batch, dim=1)
                yield predicted_class_batch, proba_batch, samples_logits_batch, samples_proba_batch, predicted_class_samples_batch, predicted_class_samples_proba, proba_samples_mean_batch
