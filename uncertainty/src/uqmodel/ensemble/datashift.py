...
from uqmodel.ensemble.ps_data import FeatureDataSet
...
class IndependentGaussianNoiseDataShift(DataShift):
    ...
    def __str__(self):
        return '{{type: IndependentGaussianNoiseDataShift, mu: {}, sigma: {}}}'.format(self.mu, self.sigma)
...
class ShiftedFeatureDataSet(FeatureDataSet):
    def __init__(self, dataset:FeatureDataSet, datashift:DataShift):
        super(ShiftedFeatureDataSet, self).__init__(dataset.df, dataset.feature_list, dataset.label_name, dataset.transform)
...
class PortionShiftedFeatureDataSet(FeatureDataSet):
    def __init__(self, dataset:FeatureDataSet, datashift:DataShift, portion:float):
        super(PortionShiftedFeatureDataSet, self).__init__(dataset.df, dataset.feature_list, dataset.label_name, dataset.transform)
        self.datashift = datashift
        ...
        self.shifted_indices = set(np.concatenate((index_0_samples, index_1_samples), axis=0))
