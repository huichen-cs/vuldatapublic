...
#    uncertainty/scripts/run_htsc_bertsap_activelearn_ext.sh 5 init
# 2. To run the active learning with ehal (entropy-based heuristic active learning) - best case
#    uncertainty/scripts/run_htsc_bertsap_activelearn_ext.sh 5 ehal_max
# 3. To run the active learning with ehal (entropy-based heuristic active learning) - worst case
#    uncertainty/scripts/run_htsc_bertsap_activelearn_ext.sh 5 elah_max
# 4. To run the active learning with random
#    uncertainty/scripts/run_htsc_bertsap_activelearn_ext.sh 5 random
...
#   python uncertainty/src/htsc_bert_sapdata_active_learn_train.py \
#     -c uncertainty/config/htscbertsap/active/en_bertsap_1.0_im_1_train_0.9_sigma_0.ini -a init
# PYTHONPATH=uncertainty/src \
#   python uncertainty/src/htsc_bert_sapdata_active_learn_train.py \
#     -c uncertainty/config/htscbertsap/active/en_bertsap_1.0_im_1_train_0.9_sigma_0.ini -a ehal
# PYTHONPATH=uncertainty/src \
#   python uncertainty/src/htsc_bert_sapdata_active_learn_train.py \
#     -c uncertainty/config/htscbertsap/active/en_bertsap_1.0_im_1_train_0.9_sigma_0.ini -a elah
###

run_config_file="uncertainty/config/htscbertsap/active/en_bertsap_1.0_im_1_train_0.9_sigma_0_ext.ini"
...
function do_action() {
	...
	echo -n "python uncertainty/src/htsc_bert_sapdata_active_learn_train_ext.py "
	echo "  -c  ${run_config_file} -i ${init_train_factor} -a ${action} -s ${seed}"
	TOKENIZERS_PARALLELISM=false PYTHONPATH=$PYTHONPATH python uncertainty/src/htsc_bert_sapdata_active_learn_train_ext.py \
		-c "${run_config_file}" -i "${init_train_factor}" -a "${action}" -s "${seed}"
}

function do_eval() {
	echo -n "PYTHONPATH=$PYTHONPATH "
	echo -n "python uncertainty/src/htsc_bert_sapdata_active_learn_eval_ext.py "
	echo "  -c ${run_config_file} -a ${eval_action}"
	TOKENIZERS_PARALLELISM=false PYTHONPATH=$PYTHONPATH python uncertainty/src/htsc_bert_sapdata_active_learn_eval_ext.py \
