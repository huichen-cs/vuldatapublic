...
class EnsembleCheckpoint(object):
    ...
    def __init__(self,
                 ...
                 warmup_epochs:int=5,
                 min_member_losses:Union[List[float],
                                         None]=None,
        ...
        if min_member_losses is None:
            self._min_member_losses = [0.]*ensemble_size
        else:
    ...
    def min_loss_updated(self,
                         index:int,
                         epoch:int,
                         loss:float) -> bool:
        logger.debug(f'Checkpoint: checking for min loss: {self._min_member_losses} loss: {loss}')
    ...
    def save_member_checkpoint(self,
                               epoch:int,
                               index:int,
    ...
    #                     optimizer_ensemble:List[
    #                         Union[
    #                             torch.optim.Adam,
    #                             torch.optim.AdamW
    #                         ]],
    #                     scheduler_ensemble:List[
    #                         torch.optim.lr_scheduler.StepLR
    #                         ],
    #                     criteria_ensemble:List[
    #                         torch.nn.CrossEntropyLoss
    #                         ],
    #                     min_loss_ensemble:List[float]
    #                     ) -> Tuple[
    #                         EnsembleBertClassifier,
    #                         List[
    #                             Union[
    #                                 torch.optim.Adam,
    #                                 torch.optim.AdamW
    ...
    #             (_,
    #              model_ensemble[i],
    #              optimizer_ensemble[i],
    #              scheduler_ensemble[i],
    #              criteria_ensemble[i],
    #              min_loss_ensemble[i]) = self.load_member_checkpoint(
    #                     i,
    #                     model_ensemble,
    #                     optimizer_ensemble,
    #                     scheduler_ensemble,
    ...
    def save_datasets(self, train_dataset:torch.utils.data.TensorDataset,
                      val_dataset:torch.utils.data.TensorDataset,
                      test_dataset:torch.utils.data.TensorDataset) -> None:
        for ds_type,ds in zip(['train', 'val', 'test'], [train_dataset, val_dataset, test_dataset], strict=True):
