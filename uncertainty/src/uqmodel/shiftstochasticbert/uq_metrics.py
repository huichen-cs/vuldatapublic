...
def brier_score(
    targets: typing.Iterable[torch.Tensor], probs: typing.Iterable[torch.Tensor]
) -> torch.Tensor:
    """
    brier score
...
def compute_accuracy_from_labels(
    test_label_pred: typing.Iterable[torch.Tensor],
    test_label: typing.Iterable[torch.Tensor],
) -> torch.Tensor:
    correct = 0
    total = 0
    for label_batch, pred_label_batch in zip(test_label, test_label_pred):
...
def compute_binary_acc_vs_conf_from_tensors(
    test_proba_pred: torch.Tensor,
    test_label: torch.Tensor,
    thresholds=torch.linspace(0, 10, 11) * 0.1,
    empty_is=1.0,
...
def compute_binary_acc_vs_conf(
    test_proba_pred: typing.Iterable[torch.Tensor],
    test_label: typing.Iterable[torch.Tensor],
    thresholds=torch.linspace(0, 10, 11) * 0.1,
    empty_is=1.0,
    ...
    acc_list, count_list = compute_binary_acc_vs_conf_from_tensors(
        test_proba_pred, test_label, thresholds=thresholds, empty_is=empty_is
    )
    return acc_list, count_list
