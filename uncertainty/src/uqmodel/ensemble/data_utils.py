...
def check_uniq_cve(patch_commit_df:pd.DataFrame) -> bool:
    ...
    for cve_group in patch_commit_df.groupby('cve'):
        ...
        if cve_df.repo.shape[0] > 1:
            logger.info('CVE ' + cve_id + ' appears in ' + ' '.join(cve_df.repo) + ', unexpected more than once')
            valid = False
        if cve_df.commit.shape[0] > 1:
            logger.info('CVE ' + cve_id + ' has patch commits ' + ', unexpected more t han one')
            valid = False
    return valid


def get_repos_patch_commits_stats(repo_commits:dict, patch_commit_df:pd.DataFrame) -> pd.DataFrame:
...
def get_patchscout_feature_list():
    ...
    #     Patch Likelihood                           The probability of a commit to be a security patch.              
            'vuln_type_1', 'vuln_type_2', 'vuln_type_3', # 'patchlike', # already in data_df
