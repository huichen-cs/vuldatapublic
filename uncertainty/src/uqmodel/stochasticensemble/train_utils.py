...
from uqmodel.stochasticensemble.ps_data import (
    FeatureDataSet, StandardScalerTransform, PsFeatureDataCollection, PsFeatureExperimentDataBuilder,
    get_dataset_stats, get_dataloader_shape
)
from uqmodel.stochasticensemble.datashift import DataShift, ShiftedFeatureDataSet, PortionShiftedFeatureDataSet
from uqmodel.stochasticensemble.stochastic_mlc import StochasticMultiLayerClassifier
from uqmodel.stochasticensemble.ensemble_mlc import StochasticEnsembleClassifier, EnsembleClassifier
...
class EarlyStopping(object):
    ...
    def __call__(self, loss):
        logger.debug(f'EarlyStopping: min loss: {self.min_loss} loss: {loss} paitence: {self.patience} counter: {self.counter}')
...
def build_datasets(config:ExperimentConfig, ckpt:EnsembleCheckpoint):
    if config.trainer.use_data == 'try_checkpoint':
        try:
            ...
            logger.info('unable to load checkpoint, prepare data set with train/test ratios: {} and validation ratio: {}'
                        .format(config.data.train_test_ratios, config.data.val_ratio))
            dataset_builder = PsFeatureExperimentDataBuilder(PsFeatureDataCollection(config.data.data_dir),
            ...
            train_dataset, val_dataset, test_dataset, ps_columns = \
                dataset_builder.train_train_dataset, dataset_builder.train_val_dataset, dataset_builder.test_dataset, \
                    dataset_builder.ps_columns
            logger.info('dataset_builder.train_train_dataset stats: {}'.format(get_dataset_stats(train_dataset)))
            logger.info('dataset_builder.train_val_dataset stats: {}'.format(get_dataset_stats(val_dataset)))
            logger.info('dataset_builder.test_dataset stats: {}'.format(get_dataset_stats(test_dataset)))
            ckpt.save_datasets(train_dataset, val_dataset, test_dataset, ps_columns)
    else:
        raise ValueError('unsupported configuration option {} for config.trainer.use_model'.format(config.trainer.use_model))
    return  train_dataset, val_dataset, test_dataset, ps_columns

def build_shifted_datasets(config:ExperimentConfig,
...
def build_dataloaders(config:ExperimentConfig,
    ...
    if config.trainer.split_data == 'sanity_check':
        ...
        logger.info('running sanity check, set train data = validation data = test data')
        ...
        logger.info('running train_validation_test job, use validation and test data set')
        ...
        raise ValueError('unsupported configuration option {} for config.trainer.split_data'.format(config.trainer.use_data))

    logger.info('train/test dataloaders are ready: train.shape: {}, val.shape: {}, test.shape: {}'
                .format(get_dataloader_shape(train_dataloader),
                        get_dataloader_shape(val_dataloader), get_dataloader_shape(test_dataloader)))
    return train_dataloader, val_dataloader, test_dataloader

def instantiate_ensemble_trainder(config:ExperimentConfig,
                                  model_type:str,
                                  ps_columns:Sequence,
                                  lr_scheduler:dict,
    ...
    if model_type == 'disentangle':
        ...
        ensemble = EnsembleClassifier(config.model.ensemble_size, len(ps_columns), 2,
                        neurons=config.model.num_neurons, dropouts=config.model.dropout_ratios,
                        activation=torch.nn.LeakyReLU())
...
def re_initialize_ensemble_trainder_from_pretrain(ensemble:Union[StochasticEnsembleClassifier, EnsembleClassifier],
...
def get_trained_model(config:ExperimentConfig,
                      ...
                      criteria:torch.nn.Module,
                      device:torch.device):
    lr_scheduler = {'T_0': config.trainer.lr_scheduler.T_0, 'T_mult': config.trainer.lr_scheduler.T_mult}
    stopper = EarlyStopping(patience=config.trainer.early_stopping.patience, min_delta=config.trainer.early_stopping.min_delta)

    ensemble, trainer = instantiate_ensemble_trainder(config,
                                                      model_type,
                                                      ps_columns,
                                                      lr_scheduler,
                                                      stopper,
    ...
    if config.trainer.use_model == 'from_pretrain':
        try:
            ...
            logger.info('retraining the ensemble model from the pretrained model (checkpoint)')
            trainer.fit(train_dataloader, val_dataloader, pin_memory=config.trainer.pin_memory)
            ensemble = trainer.load_checkpoint()
...
class SamplingFeatureDataSet(FeatureDataSet):
    def __init__(self, dataset:FeatureDataSet, ratio:float):
        super(SamplingFeatureDataSet, self).__init__(dataset.df, dataset.feature_list, dataset.label_name, dataset.transform)
        self.ratio = ratio

        n_selected = np.ceil(len(self.df) * ratio / 2).astype(int)
        index_0_list = [i for i in range(len(self.df)) if self.df.iloc[i][self.label_name] == 0]
        index_1_list = [i for i in range(len(self.df)) if self.df.iloc[i][self.label_name] == 1]
        ...
        self.df = self.df.iloc[np.sort(np.concatenate((index_0_samples, index_1_samples), axis=0)), :]
