...
class EnsembleTrainer(object):
    ...
    """
    def __init__(self, model_ensemble, criteria=None, lr_scheduler=None,
                 max_iter=500, init_lr=1e-03, device=None,
                 checkpoint=None, earlystopping=None,
                 tensorboard_log=None, **kwargs):
        self.init_trainer(model_ensemble, criteria=criteria, lr_scheduler=lr_scheduler,
                 max_iter=max_iter, init_lr=init_lr,
                 device=device,
                 checkpoint=checkpoint, earlystopping=earlystopping,
                 tensorboard_log=tensorboard_log, **kwargs)

    def init_trainer(self, model_ensemble, criteria=None, lr_scheduler=None,
                 max_iter=500, init_lr=1e-03, device=None, checkpoint=None, earlystopping=None,
                 tensorboard_log=None, **kwargs):
        ...
        if isinstance(tensorboard_log, tuple) and len(tensorboard_log) == 2:
            ...
            raise ValueError('tensorboard_log must be None or a tuple of (logdir, logtag)')
        ...
        for model in self.model_ensemble:
            if isinstance(model, StochasticMultiLayerClassifier) and 'ouput_log_sigma' not in kwargs:
                raise ValueError('Stochastic ensemble model must provide ouput_log_sigma')
        if 'ouput_log_sigma'in kwargs:
            self.output_log_sigma = kwargs['ouput_log_sigma']
            for model in self.model_ensemble:
                if self.output_log_sigma != model.output_log_sigma:
                    raise ValueError('Inconsistent output_log_sigma for Ensemble and individual stochastic models')
        ...
        for idx,model in enumerate(self.model_ensemble):
            if criteria is None:
                if isinstance(model, StochasticMultiLayerClassifier):
                    self.criteria_ensemble[idx] = StochasticCrossEntropyLoss(self.n_samples, False) # not use log_sigma
                elif isinstance(model, MultiLayerClassifier):
                    self.criteria_ensemble[idx] = torch.nn.CrossEntropyLoss()
                else:
                    raise ValueError('unsupported classifier type {}'.format(type(model)))
        ...
        self.optimizer_ensemble = [torch.optim.Adam(model.parameters(), lr=init_lr) for model in self.model_ensemble]
        if lr_scheduler is None:
        ...
        elif isinstance(lr_scheduler, dict) and set(lr_scheduler.keys()) == set(['T_0', 'T_mult']):
            self.lr_scheduler_ensemble = [
            ...
            raise ValueError('lr_scheduler must be a dict with keys [T_0, T_mult] or None')

    def fit(self, train_dataloader:torch.utils.data.DataLoader, val_dataloader:torch.utils.data.DataLoader, pin_memory=False):
        ...
        for epoch in range(self.begin_epoch, self.max_iter):
            ...
            batch_train_loss = torch.zeros(n_train_batches, self.model_size).to(self.device, non_blocking=pin_memory)
            ...
            for batch_idx, train_batch in enumerate(train_dataloader):
                ...
                for model_idx in range(self.model_size):
                    ...
                    self.lr_scheduler_ensemble[model_idx].step(epoch + batch_idx / n_train_batches)
            ...
            with torch.no_grad():
                batch_val_loss = torch.zeros(n_val_batches, self.model_size).to(self.device, non_blocking=pin_memory)
                for model_idx in range(self.model_size):
            ...
            logger.info(f'next learning rate for model -> {[lr.get_last_lr() for lr in self.lr_scheduler_ensemble]}')
            logger.info('epoch: {}, train_loss: {}, val_loss: {}, total_train_loss: {} total_val_loss: {}'.format(
                epoch, train_loss_ensemble, val_loss_ensemble, total_train_loss, total_val_loss
            ))
