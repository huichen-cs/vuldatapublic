...
help() {
	echo "Usage: run_htsc_psvcm_gn_shift_eval [-f|--n_first <n_first>]"
...
eval_from_ckpt() 
{
	...
	for ((n=_n_first; n<=_n_last; n++)); do
		...
		if [[ -z  ${_select} ]]; then
			echo "INFO: PYTHONPATH=uncertainty/src/ python uncertainty/src/htsc_ps_vcmdata_shift_${_py_key}_eval.py -c ${ini_fp}"
			if ! PYTHONPATH=uncertainty/src/ python "uncertainty/src/htsc_ps_vcmdata_shift_${_py_key}_eval.py" -c "${ini_fp}"; then
				echo "ERROR: htsc_ps_vcmdata_shift_${_py_key}_eval.py failed"
				return 1
			fi
		else
			echo "INFO: PYTHONPATH=uncertainty/src/ python uncertainty/src/htsc_ps_vcmdata_shift_${_py_key}_eval.py -c ${ini_fp} --select ${_select}"
			if ! PYTHONPATH=uncertainty/src/ python "uncertainty/src/htsc_ps_vcmdata_shift_${_py_key}_eval.py" -c "${ini_fp}" --select "${_select}"; then
				echo "ERROR: htsc_ps_vcmdata_shift_${_py_key}_eval.py failed"
				return 1
			fi	
		fi 	
