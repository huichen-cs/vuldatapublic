...
class MultiLayerClassifierHead(torch.nn.Module):
    def __init__(self, input:int, output:int=2, neurons:Sequence=[300, 300, 300], dropouts:Sequence=[0.25, 0.25, 0.25],
                 activation=None):
        ...
        layers:List[Union[torch.nn.Dropout,
                          torch.nn.Linear,
                          torch.nn.ReLU,
                          torch.nn.LeakyReLU,
                          torch.nn.Module]] = []
...
def main():
    ...
    pos = pd.read_csv(os.path.join(data_dirpath, 'SAP_full_commits.csv'), index_col=0, keep_default_na=False)
    neg = pd.read_csv(os.path.join(data_dirpath, 'SAP_negative_commits_10x.csv'), index_col=0, keep_default_na=False)
    data = create_dataset(pos,neg)

    tokenizer = AutoTokenizer.from_pretrained("microsoft/codebert-base", cache_dir=cache_loc)
    max_len = 512
    num_classes = 2
