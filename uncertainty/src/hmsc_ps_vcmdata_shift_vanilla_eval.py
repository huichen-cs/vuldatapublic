...
from uqmodel.ensemble.eval_utils import (
    add_predictive_uq_to_result_dict, VanillaUq,
...
if __name__ == '__main__':
    ...
    vallina_model = VanillaClassifier(ensemble.select_model(val_dataloader, selection_critieria=selection_criteria, device=device))

    test_proba_pred = list(vallina_model.predict_proba(test_dataloader, device=device))
    test_label_pred_list = []
    # test_conf_pred_list = []
    for _, label_bacth in vallina_model.compute_class_with_conf(test_proba_pred, device=device):
    ...
    result_dict = add_predictive_uq_to_result_dict(result_dict, VanillaUq(vallina_model, test_dataloader))
