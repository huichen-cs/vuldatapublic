...
eval_from_ckpt() 
{
	...
	for ((n=_n_first; n<=_n_last; n++)); do
		...
		ckpt_d=${ckpt_root}/en_bertsap_1.0_im_${im_ratio}_train_0.9_sigma_${_sigma}
		ini_fp=${config_root}/bertsap_1.0_im_${im_ratio}_train_0.9_sigma_${_sigma}.ini
		...
		case "${_uq_type}" in
 		...
 		if [[ -z "${_select}" ]]; then
		echo "INFO: PYTHONPATH=uncertainty/src/ python uncertainty/src/bert_sapdata_shift_${_py_key}_eval.py -c ${ini_fp}"
			if ! PYTHONPATH=uncertainty/src/ python "uncertainty/src/bert_sapdata_shift_${_py_key}_eval.py" -c "${ini_fp}"; then
				echo "ERROR: uncertainty/src/bert_sapdata_shift_${_py_key}_eval.py failed"
 				return 1
 			fi	
 		else
		echo "INFO: PYTHONPATH=uncertainty/src/ python uncertainty/src/bert_sapdata_shift_${_py_key}_eval.py -c ${ini_fp} --select ${_select}"
			if ! PYTHONPATH=uncertainty/src/ python "uncertainty/src/bert_sapdata_shift_${_py_key}_eval.py" -c "${ini_fp}" --select "${_select}"; then
				echo "ERROR: uncertainty/src/bert_sapdata_shift_${_py_key}_eval.py failed"
 				return 1
 			fi	
 		fi	
