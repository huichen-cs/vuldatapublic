...
from uqmodel.shiftstochasticbert.logging_utils import init_logging, get_global_logfilename
...
def get_dataset_size(dataset:torch.utils.data.Dataset) -> int:
    if isinstance(dataset, torch.utils.data.TensorDataset):
        ...
        raise ValueError(
            'expected torch.utils.data.TensorDataset, but encountered {}'
            .format(type(dataset)))
...
def get_train_criteria(
    ...
    if trainer_config.criteria.loss_function == 'cross_entropy_loss':
        ...
        criteria = StochasticCrossEntropyLoss(
            trainer_config.aleatoric_samples,
            use_log_sigma=False)
    else:
        raise ValueError('unsupported loss function {}'.format(
            trainer_config.criteria.loss_function
        ))
    return criteria
...
def get_train_lr_scheduler(
    ...
    if lr_scheduler_config.scheduler == 'StepLR':
        ...
        raise ValueError('unsupported lr_scheduler {}'.format(lr_scheduler_config.scheduler))
    return lr_scheduler
...
class StochasticBertTrainer(object):
    ...
    def __init__(self,
        ...
        self.lr_scheduler = get_train_lr_scheduler(trainer_config.lr_scheduler, self.optimizer)
    ...
    def _lr_scheduler_inner_step(self,
                                 n_batches:int,
                                 batch_idx:int,
                                 epoch:int):
    ...
    def _log_tb_training_summary(self,
                                 epoch:int,
                                 train_loss:float,
                                 val_loss:float,
                                 train_acc:float,
    ...
    def _train_epoch(self, epoch:int):
        ...
        for batch_idx, batch in enumerate(self.train_dataloader):
            ...
            if torch.isnan(loss):
                logger.warning('loss is nan at epoch {} for training batch_idx {}'.format(epoch, batch_idx))
        ...
        train_acc = total_train_correct / get_dataset_size(self.train_dataloader.dataset)
    ...
    def _validation_epoch(self, epoch:int):
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(self.val_dataloader):
                ...
                attention_mask = attention_mask.to(self.device, non_blocking=self.pin_memory)
                targets = targets.to(self.device, non_blocking=self.pin_memory)
                loss, logits = self._batch_validation_step(input_ids, attention_mask, targets)
                if torch.isnan(loss):
                    logger.warning('loss is nan at epoch {} for validation batch_idx {}'.format(epoch, batch_idx))
    ...
    def fit(self) -> StochasticBertBinaryClassifier:
        ...
        for epoch in tqdm_iterator:
            ...
            logger.info(
                'epoch: {}, learning_rate: {}, total_train_loss: {} total_val_loss: {} train_acc: {}, val_acc: {}'
                .format(epoch,
            ...
            if self.checkpoint and self.checkpoint.min_loss_updated(self.model_idx, epoch, total_val_loss):
        ...
        logger.info('Model {}: CUDA memory allocation {} -> {}'.format(self.model_idx, old_usage, new_usage))
...
class StochasticEnsembleTrainer(object):
    ...
    def _trainer_fit(self,
        ...
        trainer = StochasticBertTrainer(model_idx,
                              model,
                              datasets,
                              trainer_config,
                              early_stopper,
    ...
    def fit(self):
        ...
        # for model_idx,model in enumerate(self.ensemble_classifier):
            ...
            p = ctx.Process(target=self._trainer_fit,
                            args=(model_idx,
                                  model,
                                  self.datasets,
                                  self.trainer_config,
    ...
    def load_checkpoint(self) -> Tuple[StochasticEnsembleBertClassifier, float]:
        ...
        model_list:List[Union[StochasticBertBinaryClassifier, None]] = [None]*len(self.ensemble_classifier)
        for model_idx,model in enumerate(self.ensemble_classifier):
            ...
            lr_scheduler = get_train_lr_scheduler(self.trainer_config.lr_scheduler, optimizer)
            lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 5)
            (_, model, optimizer, lr_scheduler, criteria, min_member_loss) = (
                self.checkpoint.load_member_checkpoint(model_idx, model, optimizer, lr_scheduler, criteria))
