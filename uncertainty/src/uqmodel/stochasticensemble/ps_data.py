...
class FeatureDataSet(torch.utils.data.Dataset):
    def __init__(self, df:pd.DataFrame, feature_list:list, label_name:str, transform:DataTransform=None):
...
class PsFeatureExperimentDataBuilder(object):
    def __init__(self, data_collection:PsFeatureDataCollection,
                 train_test_ratios:list=(0.8, 0.2),
                 val_ratio:float=0.2,
                 imbalance_ratio:int=10,
                 cve_sample_size:Union[int, float]=100,
                 Transform:DataTransform=None, shuffle:bool=True, seed:int=None):
        self._collection = data_collection
        self._train_dataset, self._train_train_dataset, self._train_val_dataset, self._test_dataset = \
            self._get_train_val_test_dataset(train_test_ratios=train_test_ratios,
                                             val_ratio=val_ratio,
    ...
    def _get_train_val_test_cves(self,
                                 cve_sample_size:Union[int, float]=100,
                                 train_test_ratios:list=(0.8, 0.2),
                                 val_ratio:float=0.2,
                                 shuffle:bool=True,
        ...
        if isinstance(cve_sample_size, int):
            ...
            cve_list = local_cve_list[0:np.ceil(len(local_cve_list)*cve_sample_size).astype(np.int)]

        n_train_test_cves = np.cumsum(np.round(len(cve_list) * np.array(train_test_ratios)).astype(np.int))
        train_cves = cve_list[0:n_train_test_cves[0]]
        n_train_val_cves = np.cumsum(np.round(len(train_cves) * np.array([1 - val_ratio, val_ratio])).astype(np.int))
    ...
    def _get_train_val_test_splits(self, cve_splits):
        ...
        train_train_df = self._collection.df[self._collection.df['cve'].isin(train_train_cves)]
        train_val_df = self._collection.df[self._collection.df['cve'].isin(train_val_cves)]
    ...
    def _get_imbalanced_train_val_test_data(self, cve_splits, imbalance_ratio, seed=None):
        train_cves, test_cves, train_train_cves, train_val_cves = cve_splits
        _, test_df, train_train_df, train_val_df = \
            self._get_train_val_test_splits((train_cves, test_cves, train_train_cves, train_val_cves))
        train_train_df = self._get_imbalanced_data(train_train_df, train_train_cves, imbalance_ratio=imbalance_ratio, seed=seed)
        train_val_df = self._get_imbalanced_data(train_val_df, train_val_cves, imbalance_ratio=imbalance_ratio, seed=seed)
        test_df = self._get_imbalanced_data(test_df, test_cves, imbalance_ratio=imbalance_ratio, seed=seed)
        return train_train_df, train_val_df, test_df


    def _get_train_val_test_dataset(self,
        ...
        train_cves, test_cves, train_train_cves, train_val_cves = \
            self._get_train_val_test_cves(cve_sample_size=cve_sample_size,
                                          train_test_ratios=train_test_ratios,
                                          val_ratio=val_ratio,
                                          shuffle=shuffle,
                                          seed=seed)
        train_train_df, train_val_df, test_df = \
            self._get_imbalanced_train_val_test_data((train_cves, test_cves, train_train_cves, train_val_cves),
        ...
        if Transform is not None:
            ...
            transform = None
        train_dataset = FeatureDataSet(train_df, self.ps_columns, 'label', transform=transform)
        train_train_dataset = FeatureDataSet(train_train_df, self.ps_columns, 'label', transform=transform)
        train_val_dataset = FeatureDataSet(train_val_df, self.ps_columns, 'label', transform=transform)
        test_dataset = FeatureDataSet(test_df, self.ps_columns, 'label', transform=transform)
...
def get_dataloader_shape(dataloader):
    ...
    for batch in dataloader:
        X,y = batch
        total_len += len(X)
    logger.info('dataloader.shape = ({}, {}, {})'.format(total_len, X.shape[-1], len(y.shape)))
