...
from uqmodel.ensemble.eval_utils import (
    EnsembleUq,
    add_predictive_uq_to_result_dict, compute_uq_eval_metrics, result_dict_to_json
)
from uqmodel.ensemble.train_utils import (
    EnsembleCheckpoint, SamplingFeatureDataSet,
    build_datasets, build_shifted_datasets, build_dataloaders, get_trained_model
)
...
def get_focal_loss(config:ExperimentConfig, train_dataset, device=None):
    focal_alpha = compute_focalloss_alpha(train_dataset, 2, config.data.imbalance_ratio, device)
...
if __name__ == '__main__':
    ...
    if config.data.sample_from_train and config.data.sample_from_train < 1.0:
        train_dataset = SamplingFeatureDataSet(train_dataset, config.data.sample_from_train)
        logger.info('training dataset sampled with ratio {} results in from {} to {}'
                    .format(config.data.sample_from_train, train_len, len(train_dataset)))    
    train_dataset, val_dataset, test_dataset = build_shifted_datasets(config, train_dataset, val_dataset, test_dataset)
    train_dataloader, val_dataloader, test_dataloader = build_dataloaders(config, train_dataset, val_dataset, test_dataset)
    focal_loss_ensemble = [get_focal_loss(config, train_dataset, device=device) for _ in range(config.model.ensemble_size)]
