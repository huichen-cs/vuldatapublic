...
if [ -z ${runid} ]; then
  echo "runid is unset"
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext.sh 5 elah_max; then
    echo "Failed to run elah_max."
    exit 1
  fi
else
  echo "runid is set to '${runid}'"
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext.sh 5 elah_max "${runid}"; then
