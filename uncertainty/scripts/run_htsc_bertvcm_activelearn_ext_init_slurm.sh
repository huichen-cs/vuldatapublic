...
if [ -z ${runid} ]; then
  echo "runid is unset"
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext.sh 5 init; then
    echo "Failed to run init."
    exit 1
  fi
else
  echo "runid is set to '${runid}'"
  if ! uncertainty/scripts/run_htsc_bertvcm_activelearn_ext.sh 5 init "${runid}"; then
    echo "Failed to run init with runid '${runid}'."
    exit 1
  fi
fi
