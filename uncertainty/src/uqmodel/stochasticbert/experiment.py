...
class ExperimentConfig(object):
    ...
    class TrainerConfig(object):
        ...
        _USE_DATA = set(
            ["use_checkpint", "try_checkpoint"]
        )  # how models are used for training
        _USE_MODEL = set(
            ["use_checkpint", "try_checkpoint", "from_pretrain", "from_scratch"]
        )
        ...
        class Criteria(object):
            ...
            def update_from_config(self, config):
                ...
                if self.loss_function == "cross_entropy_loss" and hasattr(
                    self, "focal_gamma"
                ):
                    delattr(self, "focal_gamma")
                if self.loss_function == "stochastic_cross_entropy_loss" and hasattr(
                    self, "focal_gamma"
                ):
                    delattr(self, "focal_gamma")
            ...
            def __str__(self):
                if self.loss_function == "cross_entropy_loss":
                    ...
                    return "{{loss_function: {}, focal_gamma: {}}}".format(
                        self.loss_function, self.focal_gamma
                    )
                elif self.loss_function == "stochastic_cross_entropy_loss":
                    return "{{loss_function: {}}}".format(self.loss_function)
                else:
                    raise ValueError(
                        "unsupported loss function {}".format(self.loss_function)
                    )
