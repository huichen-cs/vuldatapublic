...
from uqmodel.stochasticbert.sampling_metrics import compute_sampling_entropy, compute_sampling_mutual_information
...
from uqmodel.stochasticbert.datashift import DataShift, ShiftedFeatureDataSet, PortionShiftedFeatureDataSet
...
def load_from_checkpoint_with_datashift(model_type:str, config:ExperimentConfig, device:torch.DeviceObjType=None):
    ...
    try: 
        ...
        raise ValueError(
            ...
            .format(config.trainer_checkpoint_dir_path)) from err
...
class EnsembleDisentangledUq(object):
    ...
    def _fit(self, model_idx, batch_idx, model, input_ids, attention_mask, device=None):
        ...
        if device is None:
            ...
            mu, sigma = self._cpu_batch_fit(model_idx, batch_idx, model, input_ids, attention_mask)
            return mu, sigma

    def _batch_fit(self, model_idx, batch_idx, model, input_ids, attention_mask, logfilename, rtn_dict):
    ...
    def _mp_fit(self, model_idx, batch_idx, model, input_ids, attention_mask, device=None):
        torch.cuda.empty_cache()
        if device is None:
            ...
            p = ctx.Process(target=self._batch_fit,
                            args=(model_idx,
                                  batch_idx,
                                  model,
                                  input_ids,
    ...
    def _slow_compute_uq_logits(self):
        ...
        for batch_idx, batch in enumerate(self.data_loader):
            ...
            for model_idx, model in enumerate(self.ensemble):
                ...
                # model.train()
                (mu_model_list[model_idx], sigma_model_list[model_idx]) = (
                    self._mp_fit(model_idx, batch_idx, model, input_ids, attention_mask)
                )
    ...
    def _mp_member_model_logits(self, dataloader, model, n_batches, device):
        ...
        rtn_dict = manager.dict()
        ctx = mp.get_context('spawn')
        p = ctx.Process(target=self._member_model_logits,
    ...
    def _mp_member_model_logits_q(self, dataloader, model, n_batches, device):
        ...
        queue = manager.Queue()
        ctx = mp.get_context('spawn')
        p = ctx.Process(target=self._member_model_logits_q,
        ...
        while not queue.empty():
            ...
            mu_batch_list[batch_idx], sigma_batch_list[batch_idx] = mu.detach().cpu(), sigma.detach().cpu()            
        mu_batches = torch.cat(mu_batch_list, dim=0)
        sigma_batches = torch.cat(sigma_batch_list, dim=0)
        return mu_batches, sigma_batches
        ...
        for model_idx, model in enumerate(self.ensemble):
            ...
            for batch_idx, batch in enumerate(self.data_loader):
                ...
                if self.mp:
                    ...
                    (mu_batch_list[batch_idx], sigma_batch_list[batch_idx]) = (
                        self._fit(model_idx, batch_idx, model, input_ids, attention_mask)
                    )
            mu_batches = torch.cat(mu_batch_list, dim=0)
            sigma_batches = torch.cat(sigma_batch_list, dim=0)
            mu_model_list[model_idx] = mu_batches
