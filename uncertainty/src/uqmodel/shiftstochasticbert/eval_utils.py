...
from .sampling_metrics import compute_sampling_entropy, compute_sampling_mutual_information
...
class StochasticEnsembleBertClassifierEvalautor(object):
    ...
    def _predict_proba(self, dataloader:torch.utils.data.DataLoader, n_samples:int, device:torch.device):
         with torch.no_grad():
            for batch in dataloader:
                ...
                for idx in range(len(self.ensemble)):
                    ...
                    proba = self.ensemble[idx].predict_sampling_proba(input_ids, attention_mask, n_samples)
    ...
    def compute_ece(self, predicted_proba_tensor, test_label_tensor):
        if version.parse(torchmetrics.__version__) > version.parse('0.11.4'):
            ...
            updated_predicted_proba_tensor = self.set_zero_to_nextafter(predicted_proba_tensor[:, 1].contiguous())
    ...
    def compute_uq_eval_metrics(self,
                                config,
                                proba_pred,
                                labels_pred,
                                targets,
        ...
        cmtx = torchmetrics.functional.classification.binary_confusion_matrix(labels_pred, targets)
       
        proba_contiguous = proba_pred[:, 1].contiguous()
        prc_precision, prc_recall, prc_thresholds = torchmetrics.functional.classification.binary_precision_recall_curve(proba_contiguous, targets, thresholds=20)
        prc = self.curve_triplet_to_dict((prc_precision, prc_recall, prc_thresholds), ('precision', 'recall', 'thresholds'))

        auprc = torchmetrics.functional.classification.binary_average_precision(proba_contiguous, targets, thresholds=None).cpu().item()
        roc_fpr, roc_tpr, roc_thresholds = torchmetrics.functional.classification.binary_roc(proba_contiguous, targets, thresholds=20)
        roc = self.curve_triplet_to_dict((roc_fpr, roc_tpr, roc_thresholds), ('fpr', 'tpr', 'thresholds'))
        
        auroc = torchmetrics.functional.classification.binary_auroc(proba_contiguous, targets, thresholds=None).cpu().item()
        
        ece = self.compute_ece(proba_pred, targets)
        score = brier_score_from_tensors(self.get_one_hot_label(targets, num_classes=2), proba_pred)
        conf_thresholds = torch.linspace(0, 10, 11)*0.1
        if metrics_list:
            metrics_dict, count_list = compute_binary_metrics_vs_conf_from_tensors(proba_pred, targets, thresholds=conf_thresholds, metrics_list=metrics_list)
        else:
            acc_list, count_list = compute_binary_acc_vs_conf_from_tensors(proba_pred, targets, thresholds=conf_thresholds)
    ...
    def compute_eval_metrics(self, config:ExperimentConfig):
        ...
        probas_pred_list = list(self._predict_mean_proba(self.dataloader,
        ...
        result_dict = self.compute_uq_eval_metrics(config,
                                            probas_pred,
                                            labels_pred,
                                            targets,
        ...
        uq_list = []
        uq = EnsembleDisentangledUq(self.ensemble, self.dataloader, config.trainer.aleatoric_samples, device=config.device)
        (
            proba_std_aleatoric,
            proba_mean_aleatoric,
            entropy_aleatoric,
