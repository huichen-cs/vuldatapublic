...
from uqmodel.stochasticbert.logging_utils import get_global_logfilename, init_logging
...
def get_trained_ensemble_model(
    ...
    logger.info(
        "N(run_dataset): %d, N(val_dataset): %d",
...
def result_to_json(result_dict):
    for k in result_dict:
        ...
        if isinstance(result, list):
            ...
            raise ValueError(f"unsupported data type {type(result)}")
    result_json = result_dict_to_json(result_dict)
    return result_json
...
def _mp_run_one_evaluation(config, run_datasets, queue, logfilename):
    ...
    try:
        ...
        logger.info("load ensemble model: %s", repr(err))
        queue.put(None)
...
def _run_one_evaluation(config, run_datasets):
    ...
    eval_metrics = compute_eval_metrics(
        ensemble, run_dataloaders.test_dataloader, config=config
    )
...
def print_allocation(title, disp):
    nbytes = torch.cuda.memory_allocated(torch.device("cuda"))
    disp(f"{title}: CUDA memory allocated: {nbytes} bytes")
    ...
    for obj in gc.get_objects():
        try:
            if torch.is_tensor(obj) or (
                ...
                print(f"{str(obj):<20}", type(obj), obj.size())
        except Exception:
            pass
...
def run_eval_experiment(config: ExperimentConfig) -> dict:
    ...
    for method in get_active_learning_method_list():
        if config.action in (method, "all"):
            ...
            logger.info(
                "begin %s with len(run_dataset): %d, len(pool_dataset): %d",
                    method,
            ...
            for i in range(n_data_parts):
                try:
                    ...
                    run_datasets.update_checkpoint(f"{i}_{method}")
                    eval_metrics = run_one_evaluation(config, run_datasets)
                    if not eval_metrics:
                        logger.info("cannot evaluate for method %s", method)
                        raise FileNotFoundError(
                            "model for method " + method + " not found"
                        )
                    # run_dataloaders = BertExperimentDataLoaders(config, run_datasets, train=False)
                    ...
                    logger.info("done %s at %d", method, i)
                    ...
                    logger.info(
                        "result for %s at %d does not exist, due to %s", method, i, err
                    )
            result_dict[method] = result_list
            result_dict[f"{method}_data_increment"] = data_increment_list
    return result_dict
...
if __name__ == "__main__":
    ...
    exp_result_dict = run_eval_experiment(exp_config)
    json = result_to_json(exp_result_dict)
    print(json)
    # print("done")
