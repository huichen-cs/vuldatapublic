...
class StochasticBertBinaryClassifier(nn.Module):
    def __init__(self, num_classes=2, dropout_prob=0.1, cache_dir="~/.hfcache"):
        ...
        output_mu_dict["output_mu"] = torch.nn.Linear(
...
class StochasticBertBinaryClassifierTrainer(object):
    ...
    def step(self):
        ...
        for input_ids, attention_mask, targets in tqdm(
            self.train_loader, desc="train_step", disable=self.disable_tqdm
        ):
    ...
    def train(self, num_epochs):
        for epoch in tqdm(range(num_epochs), desc="train", disable=self.disable_tqdm):
            ...
            print(
                f"Epoch {epoch+1}: Train loss: {train_loss:.4f} Accuracy: {accuracy: .4f}"
            )


class StochasticBertBinaryClassifierEvaluator(object):
    @classmethod
    def batch_stochastic_softmax(
        self,
        mu,
        sigma_log_sigma,
        n_samples,
        passed_log_sigma=True,
    ...
    def batch_stochastic_pass(self, input_ids, attention_mask):
        mu_samples = torch.zeros(
            self.n_stochastic_passes, len(input_ids), self.model.output_shape[-1]
        )
        sigma_samples = torch.zeros(
            self.n_stochastic_passes, len(input_ids), self.model.output_shape[-1]
        )
        for i in tqdm(
            range(self.n_stochastic_passes),
            desc="dropout_passes",
    ...
    def evaluate(self, test_loader):
        ...
        for batch in tqdm(test_loader, desc="eval", disable=self.disable_tqdm):
            ...
            mu_samples, sigma_samples = self.batch_stochastic_pass(
                input_ids, attention_mask
            )
            proba_passes = torch.zeros(
                self.n_stochastic_passes, len(input_ids), *(self.model.output_shape[1:])
            )
            for i, (mu, sigma) in tqdm(
                enumerate(zip(mu_samples, sigma_samples)),
                desc="eval_softmax",
                disable=self.disable_tqdm,
            ):
                _, proba = self.batch_stochastic_softmax(
                    mu,
                    sigma,
