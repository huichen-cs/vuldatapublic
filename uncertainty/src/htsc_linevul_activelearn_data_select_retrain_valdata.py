...
class LineVulExperimentConfig:
    ...
    def __init__(self):
        ...
        self.val_file = os.path.join("data", "linevul", "linevul_val_data.csv")
    ...
    def from_cmdline(self, args: argparse.Namespace):
        ...
        self.val_file = args.linevul_val_file
...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--linevul_val_file", type=str, help="Input file containing validation data"
    )
    parser.add_argument(
        "--output_file_dir",
        type=str,
...
def run_data_select_experiment(
    ...
    logger.info(
        (
            "initialized linevul_experiment_datasets from checkpoint ->  "
            "N(train_dataset)=%d "
            "N(val_dataset)=%d "
            "N(run_dataset)=%d "
            "N(pool_dataset)=%d"
        ...
        len(linevul_experiment_datasets.val_dataset),
    ...
    linevul_val_dataset = TextClassificationDataset(
            pd.read_csv(linevul_config.val_file, na_filter=False),
            linevul_experiment_datasets.val_dataset.tokenizer,
            linevul_experiment_datasets.val_dataset.max_len,
            linevul_experiment_datasets.val_dataset.num_classes,
        )
    val_dataset = deepcopy(
        torch.utils.data.ConcatDataset(
            [experiment_datasets.val_dataset, linevul_val_dataset]
        )
    )
    linevul_experiment_datasets.val_dataset = val_dataset
    logger.info(
        (
            "updated linevul_experiment_datasets for linevul ->  "
            "N(train_dataset)=%d "
            "N(val_dataset)=%d "
            "N(run_dataset)=%d "
            "N(pool_dataset)=%d"
