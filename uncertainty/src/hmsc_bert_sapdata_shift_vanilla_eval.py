...
from uqmodel.shiftbert.eval_utils import VanillaBertClassifierEvalautor, EnsembleModelSelector
...
def main():
    init_logging(__file__, append=True, config_file=os.path.join('logconf', 'logger_mp_bertps_shift.yml'))

    config, selection_criteria = setup_experiment()
    logger.info('Evaluate dropout UQ model for ensemble member selected via {}'.format(selection_criteria))
    ...
    vanilla_model = member_selector.select_member_model(selection_critieria=selection_criteria)
    torch.cuda.empty_cache() # only one member model needed on CUDA
    evaluator = VanillaBertClassifierEvalautor(config, vanilla_model, datasets.test_dataset)
