...
class EnsembleCheckpoint(object):
    ...
    """
    def __init__(self, checkpoint_dir_path, warmup_epochs=5, min_loss=None, min_member_loss=None, tag=None, train=True):
    ...
    def min_loss_updated(self, epoch, loss):
        logger.debug(f'Checkpoint: checking for min loss: {self._min_member_loss} loss: {loss}')
        if epoch < self._warmup_epochs:
    ...
    def save_member_checkpoint(self, index, model, optimizer, scheduler, criteria, min_member_loss, loss):
    ...
    def save_datasets(self, train_dataset:torch.utils.data.Dataset,
    ...
    def load_datasets(self) -> Tuple[torch.utils.data.Dataset,
