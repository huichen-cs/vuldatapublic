...
class StochasticMultiLayerClassifier(torch.nn.Module):
    def __init__(self, input:int, output_log_sigma:bool, output:int=2, neurons:Sequence=[300, 300, 300], dropouts:Sequence=[0.25, 0.25, 0.25],
                 *, normalization_layers=False, activation=None):
        ...
        for i in range(1, len(neurons)):
            ...
            shared_layer_dict['hidden_' + str(i)] = torch.nn.Linear(neurons[i-1], neurons[i])
            if normalization_layers:
                shared_layer_dict['batch_norm_' + str(i)] = torch.nn.BatchNorm1d(neurons[i])
            self._add_activiation_layer(shared_layer_dict, 'activation_' + str(i), activation)

        # output layer module
        if dropouts[len(neurons)-1] is not None:
            output_mu_dict['dropout_mu_' + str(len(neurons)-1)] = torch.nn.Dropout(dropouts[len(neurons)-1])
            output_sigma_dict['dropout_sigma_' + str(len(neurons)-1)] = torch.nn.Dropout(dropouts[len(neurons)-1])
        ...
        self._add_activiation_layer(output_mu_dict, 'activation_mu_' + str(len(neurons)-1), activation)
    
        output_sigma_dict['output_sigma'] = torch.nn.Linear(neurons[len(neurons)-1], output)
        # output_sigma_dict['batch_norm_sigma'] = torch.nn.BatchNorm1d(output)
        if self._output_log_sigma:
            self._add_activiation_layer(output_sigma_dict, 'activation_sigma_' + str(len(neurons)-1), activation)
        else:
            self._add_activiation_layer(output_sigma_dict, 'activation_sigma_' + str(len(neurons)-1), 'softplus')
    ...
    def _add_activiation_layer(self, shared_layer_dict:collections.OrderedDict, name:str, activation: Union[str, torch.nn.Module]):
